    ...
    try
    {
        while (m_scheduler->has_ready_threads())
        {
            ...
            if(status == MP_Thread::FINISHED){
                mp_memman->deallocate(current_thread);
            }
...
void * MP_OS::mp_malloc(numbytes){
    MP_Thread *currentThread = dispatcher.GetCurrentThread();
    mp_memman->allocate(numbytes,currentThread);
}
